...
int sendudp(int sock, unsigned long *saddr, unsigned long *daddr, unsigned int sport, unsigned int dport, char *data, int len)
{
	...
	*/
	packet = (char *)malloc(sizeof(struct ip)+sizeof(struct udphdr)+len);
	memset(packet,0,sizeof(struct ip) + sizeof(struct udphdr) + len);
	...
	*/
	...
	iph->ip_hl = 5;
	iph->ip_ttl = 255;
	iph->ip_id = htons((unsigned short) rand());
	...
	dstaddr.sin_family = AF_INET;
	dstaddr.sin_addr.s_addr = *daddr;
	if (sendto(sock, packet, sizeof(struct ip) + sizeof(struct udphdr)+len,0,(struct sockaddr *)&dstaddr,sizeof(struct sockaddr_in)) < 0)
